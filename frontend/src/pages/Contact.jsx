...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://arta-frontend-hcl1.vercel.app/api/contact', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 py-24 px-6">
      <div className="max-w-4xl mx-auto text-white">
        ...
        {submitted && (
          <div className="bg-green-800/60 rounded-xl p-6 text-center text-green-300 font-medium shadow-lg mb-8">
            ✅ Thank you for reaching out! We will get back to you shortly.
          </div>
        )}

        {errorMessage && (
          <div className="bg-red-800/60 rounded-xl p-6 text-center text-red-300 font-medium shadow-lg mb-8">
            ❌ {errorMessage}
          </div>
        )}
        ...
        >
          ...
          >
            {isSubmitting ? 'Sending...' : 'Send Message'} <Send size={22} />
          </button>
        </form>
      </div>
    </div>
