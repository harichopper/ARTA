...
const sampleAuctions = [
  {
    ...
    currentBid: 2500,
    endTime: new Date(Date.now() - 3600 * 1000), // expired 1 hour ago
  },
  {
    id: 2,
    ...
    image: 'https://images.unsplash.com/photo-1503602642458-232111445657?auto=format&fit=crop&w=400&q=80',
    currentBid: 1500,
    endTime: new Date(Date.now() - 7200 * 1000), // expired 2 hours ago
  },
  {
    id: 3,
    ...
    currentBid: 350,
    endTime: new Date(Date.now() + 1800 * 1000), // still active
  },
];
...
export default function PastAuctions() {
  ...
  // Filter only expired auctions (endTime <= now)
  const expiredAuctions = auctions.filter(a => new Date(a.endTime) <= new Date());

  useEffect(() => {
    const timer = setInterval(() => {
      const newTimeLeft = {};
      expiredAuctions.forEach(({ id, endTime }) => {
      ...
      <div className="max-w-7xl mx-auto text-white p-10 pb-24">
        ...
        {expiredAuctions.length === 0 ? (
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {expiredAuctions.map(({ id, title, image, currentBid, endTime }) => (
              <div
                key={id}
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <p className="text-red-500 font-semibold mb-2">Status: Inactive</p>
                  <p className="text-gray-300 flex-grow text-lg">{timeLeft[id] || "Auction ended"}</p>
                </div>
              </div>
            ))}
          </div>
